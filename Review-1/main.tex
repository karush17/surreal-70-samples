...
The work presents a unified objective for divergence minimizaiton consisting of action and perception stages. The perception stage aims to align the latent beliefs of the agent with its inputs, while the action stage soughts to align future inputs with the latent beliefs. The agent makes use of the two stages in order to approximate a distribution over its latent variables with respect to a desired target distribution. The unified objective serves as generalization for retrieving various RL objectives with the requirement of task rewards rendered as an optional metric. The unified objective serves as a general recipe for devising novel information-based objectives but does not yield an application for the same. The work paves the way for utilizing agent dynamics in latent space and its practical application to real-world RL applications.   
